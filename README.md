...
KTX 열차 조회에는 두 가지 독립적인 옵션이 있습니다.
- `인접역 포함 조회`: 코레일 서버의 인접역(adjStnScdlOfrFlg) 조회를 켜고, 추가로 서울/용산/수서처럼
  하나로 묶인 역 그룹은 각 역으로 직접 다시 조회해 결과를 합쳐서 보여줍니다. 서버의 인접역 플래그만으로는
  서울/용산-수서 조합이 항상 나오지 않는 것으로 확인되어(예: 서울→진주 조회 시 수서 출발이 빠짐) 이렇게
  보강했습니다.
